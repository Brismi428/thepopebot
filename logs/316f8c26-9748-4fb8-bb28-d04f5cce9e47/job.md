...
Problem description:
- Monitor one URL for uptime status
- Check every hour automatically
- Log timestamp, status code, and response time to a CSV file
- Keep it simple - no external APIs needed
- Basic error handling for timeouts/failures
- Output format: CSV with columns timestamp, status_code, response_time_ms, status (up/down)
